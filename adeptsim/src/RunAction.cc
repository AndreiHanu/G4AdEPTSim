...
void RunAction::RecordEventPos(G4ThreeVector evtPos)
{
	...
	projXZ[GetBin(evtPos.z(),-500.00,500.00,2500)][GetBin(evtPos.x(),-500.00,500.00,2500)] += 1;
	
	// Increment Y-Z projection
	projYZ[GetBin(evtPos.z(),-500.00,500.00,2500)][GetBin(evtPos.y(),-500.00,500.00,2500)] += 1;
}

void RunAction::EndOfRunAction(const G4Run* run)
{
	// Initialize OpenCV Mat object using the projection data
	Mat XZ = Mat(2500, 2500, CV_32SC1, projXZ);
	Mat YZ = Mat(2500, 2500, CV_32SC1, projYZ);
	...
	if (sum(XZ)[0] > 0. && sum(YZ)[0] > 0.){
		...
		G4String eventFileXZ_mat = "event_" + G4UIcommand::ConvertToString(run->GetRunID()) + "_" + 
								particleGun->GetGPS()->GetParticleDefinition()->GetParticleName() + 
								"_XZ_.xml";

		G4String eventFileYZ_mat = "event_" + G4UIcommand::ConvertToString(run->GetRunID()) + "_" + 
								particleGun->GetGPS()->GetParticleDefinition()->GetParticleName() + 
								"_YZ_.xml";

		FileStorage file(eventFileXZ_mat,FileStorage::WRITE);
		FileStorage file1(eventFileYZ_mat,FileStorage::WRITE);
